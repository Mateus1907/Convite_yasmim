...
const DATA_CASAMENTO = new Date(2026, 10, 7, 16, 30, 0);
...
const descricaoConvite = document.getElementById("descricaoConvite");
...
const tipoConvite = document.getElementById("tipoConvite");
...
function atualizarContador() {
    ...
    const distancia = DATA_CASAMENTO - agora;
...
copiarPix.addEventListener("click", async () => {
    try {
        await navigator.clipboard.writeText(pix.innerText);
        pixMensagem.innerHTML = "Chave PIX copiada!";
        setTimeout(() => { pixMensagem.innerHTML = ""; }, 3000);
    } catch {
...
function processarResposta(dados) {
    if (!dados) {
        mostrarMensagem("Resposta inválida.", "erro");
        return;
    }

    if (dados.status === "erro") {
        mostrarMensagem(dados.mensagem || "Convite não encontrado.", "erro");
        return;
    }

    if (dados.status === "confirmado") {
        mostrarMensagem("Este convite já foi confirmado.", "aviso");
        return;
    }
    ...
    descricaoConvite.innerHTML = "Selecione quem irá comparecer ao casamento.";
    tipoConvite.innerHTML = dados.tipo || "Família";
...
function mensagemTemporaria(texto, tipo, tempo = 3500) {
...
function criarListaConvidados() {
    ...
    pessoas.forEach((nome, index) => {
        ...
        item.querySelector("input").addEventListener("change", atualizarQuantidade);
    });
}
...
function atualizarQuantidade() {
    ...
    const checkboxes = listaConvidados.querySelectorAll("input");

    checkboxes.forEach((checkbox) => {
        if (checkbox.checked) {
            confirmados.push(checkbox.value);
        }
    });

    totalSelecionados.innerHTML = confirmados.length;
}
...
async function confirmarPresenca() {
    ...
    if (confirmados.length === 0) {
        mostrarMensagem("Selecione pelo menos uma pessoa.", "erro");
        return;
    }
    ...
    try {
        ...
        if (retorno.status === "ok") {
            ...
            listaConvidados.querySelectorAll("input").forEach((input) => {
                input.disabled = true;
            });
        ...
        mostrarMensagem(retorno.mensagem || "Não foi possível confirmar.", "erro");
        liberarBotoes();
    } catch (error) {
        fecharLoading();
        liberarBotoes();
        console.error(error);
        mostrarMensagem(error.message || "Erro ao confirmar presença.", "erro");
    }
}

/* ==========================================================
   SALVAR / LIMPAR CONVITE NO NAVEGADOR
   (lembra o último nome pesquisado ao recarregar a página)
========================================================== */

function salvarConviteLocal() {
    if (!conviteAtual) return;
    localStorage.setItem("conviteAtual", conviteAtual);
    localStorage.setItem("representante", representante.value.trim());
}
...
window.addEventListener("load", () => {
    ...
    const ultimoRepresentante = localStorage.getItem("representante");
...
function animarLista() {
    const itens = listaConvidados.querySelectorAll(".convidado");
...
observer.observe(listaConvidados, { childList: true });
...
window.addEventListener("offline", () => {
    mostrarMensagem("Você está sem conexão com a internet.", "erro");
});
...
/* ==========================================================
   DESABILITAR / LIBERAR BOTÕES ENQUANTO ENVIA
========================================================== */
...
function validarNome() {
    ...
    if (nome.length < 3) {
        mensagemTemporaria("Digite um nome válido.", "erro");
        return false;
    }
...
representante.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
        e.preventDefault();
        if (validarNome()) {
            buscarConvite();
